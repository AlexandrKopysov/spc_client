...
type ParamUI = IParamDTO & {
  isNew?: boolean;
};

export default defineComponent({
  name: 'DirectoryParameters',
  mixins: [dictMixis<ParamUI>()],
  setup() {
    return {
      storeActoinUpsertRow: 'parameterState/upsert',
      storeActoinDeleteRow: 'parameterState/delete',
    };
  },
  data() {
    return {
      rows: [] as ParamUI[],
      editingRows: {} as Record<string, ParamUI>,
      selectionsRowData: [] as ParamUI[],
  ...
  methods: {
    ...
    loadLazyData() {
      this.loading = true;
      this.$store
        .dispatch('parameterState/getLazy', this.lazyParams)
        .then((response) => {
